...
var sierpinskiTriangle = function () {
	'use strict';
 ...
 */
	...
	function drawSierpinskiTriangleFlower(paper, centerPoint, size, level) {
		...
		var height = fractalsUtil.getTriangleHeight(2 * Math.PI / 6, sideLength);
 ...
 */
	function getSierpinskiTriangle(centerPoint, sideLength, options) {
		...
		if (!(orientation == -1 || orientation == 1)) orientation = 1;
 ...
 */
	function iterativeSierpinskiTriangleRoutine(centerPoint, sideLength, level, options) {
		...
		while (triangleQueue.length < Math.pow(3, level)) {
			...
			nextTriangle = triangleQueue.shift();
			centerPoint = nextTriangle.centerPoint;
			sideLength = nextTriangle.sideLength;

			// draw next triangle
			var height = fractalsUtil.getTriangleHeight(fractalsUtil.RADIANS_60_DEGREES, sideLength);
 ...
 **/
	function getEquilateralTrianglePathList(startPoint, sideLength, orientation) {
		...
		var pathList = [['M', startPoint.X, startPoint.Y]];
		var angle = 0;
		var point = startPoint;
		var angleChangeRadians = 2 * Math.PI / 3; // adding 120 degrees
		for (var i = 0; i < 3; i++) {
			point = fractalsUtil.getNextPoint(point, sideLength, angle);
			pathList = fractalsUtil.addLine(pathList, point);
	...
	// Exports
	return {
		drawSierpinskiTriangleFlower: drawSierpinskiTriangleFlower,
		drawSierpinskiTriangle: drawSierpinskiTriangle,
		getSierpinskiTriangle: getSierpinskiTriangle
	};
}();