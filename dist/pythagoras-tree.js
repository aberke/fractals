 ...
 **/
	function pythagorasTreeRoutine(pathList, centerPoint, sideLength, level, orientation, edgePathFunction) {
		...
		var leftChildCenterPoint = {
			X: centerPoint.X + (1 / 2 * tightener * sideLength * Math.cos(orientation) + 3 / 4 * tightener * sideLength * Math.cos(leftChildOrientation)),
			Y: centerPoint.Y + (1 / 2 * tightener * sideLength * Math.sin(orientation) + 3 / 4 * tightener * sideLength * Math.sin(leftChildOrientation))
		};
		var rightChildCenterPoint = {
			X: centerPoint.X + (1 / 2 * tightener * sideLength * Math.cos(orientation) + 3 / 4 * tightener * sideLength * Math.cos(rightChildOrientation)),
			Y: centerPoint.Y + (1 / 2 * tightener * sideLength * Math.sin(orientation) + 3 / 4 * tightener * sideLength * Math.sin(rightChildOrientation))
		};
 ...
 */
	function pythagorasTreeSquare(centerPoint, orientation, sideLength, edgePathFunction) {
		...
		["M", bottomLeftPoint.X, bottomLeftPoint.Y], edgePathFunction(bottomLeftPoint, bottomRightPoint, centerPoint), edgePathFunction(bottomRightPoint, topRightPoint, centerPoint), edgePathFunction(topRightPoint, topLeftPoint, centerPoint), edgePathFunction(topRightPoint, bottomLeftPoint, centerPoint)];
	}
