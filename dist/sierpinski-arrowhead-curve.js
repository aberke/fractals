...
// define globals for JS linting
/* globals fractalsUtil */

var sierpinskiArrowheadCurve = function () {
	'use strict';
 ...
 */
	...
	function getSierpinskiArrowheadCurve(centerPoint, sideLength, options) {
		...
		if (level % 2 === 0) {
			...
			// level is odd
			angle -= fractalsUtil.RADIANS_60_DEGREES;
			arrowheadCurve(level, pathList, startPoint, sideLength, angle, 1, orientation);
		}
		return pathList;
	}
 ...
 */
	function arrowheadCurve(level, pathList, fromPoint, sideLength, angle, angleChange, orientation) {
		...
		if (level === 0) {
			var nextPoint = fractalsUtil.getNextPoint(fromPoint, sideLength, orientation * angle);
			// appends to pathList
			pathList = fractalsUtil.addLine(pathList, nextPoint);
			last = {
				point: nextPoint,
				angle: angle
			};
		...
		// turn
		angle += angleChange * fractalsUtil.RADIANS_60_DEGREES;
	...
	// Exports
	return {
		getSierpinskiArrowheadCurve: getSierpinskiArrowheadCurve
	};
}();